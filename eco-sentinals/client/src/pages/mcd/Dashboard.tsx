...
export default function Dashboard() {
  ...
  // 1. Fetch Wards
  const { data: wardsData } = useAuthGuardedQuery<Ward[]>(
    async () => {
  ...
  const { data: alertsData, fetching: fetchingAlerts } = useAuthGuardedQuery<Alert[]>(
    async () => {
  ...
  const { data: readingsData, fetching: fetchingReadings } = useAuthGuardedQuery<Reading[]>(
    async () => {
