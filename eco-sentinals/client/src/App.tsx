...
function HomeRedirect() {
  const { session, role, isReady, isNewUser } = useAuth();
  ...
  if (role === 'mcd') return <ErrorBoundary><Dashboard /></ErrorBoundary>;
  if (role === 'citizen') return <ErrorBoundary><Home /></ErrorBoundary>;
    ...
    <BrowserRouter>
      <AuthProvider>
        <Routes>
          ...
          {/* Direct Role Routes (Aliases) */}
          <Route path="/citizen-app" element={<HomeRedirect />} />
          <Route path="/mcd-dashboard" element={<HomeRedirect />} />
